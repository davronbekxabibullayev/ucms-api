...
public record GetFilteredIncomesMessage(
    ...
    string? Query,
    DateTime? From,
    DateTime? To) : IRequest<PagedResult<IncomeModel>>;
...
public class GetFilteredIncomesConsumer : RequestHandler<GetFilteredIncomesMessage, PagedResult<IncomeModel>>
{
    ...
    protected override async Task<PagedResult<IncomeModel>> Handle(GetFilteredIncomesMessage message,
        CancellationToken cancellationToken)
    {
        return await GetIncomes(message, cancellationToken);
    }

    private async Task<PagedResult<IncomeModel>> GetIncomes(GetFilteredIncomesMessage message, CancellationToken cancellationToken)
    {
        ...
        return await query.OrderByDescending(a => a.IncomeDate)
                          .ToPagedResultAsync<Income, IncomeModel>(message.Paging, _mapper, cancellationToken);
    }
}
